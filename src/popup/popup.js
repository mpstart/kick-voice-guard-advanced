// src/popup/popup.js - v3.1 - TODOS LOS CONTROLES 100% FUNCIONALES
let currentSamples = [];

// Función completa para crear fingerprint (MFCC-like)
async function createFingerprintFromBuffer(audioBuffer) {
  ...
  const samples = [];
  const interval = 40;
  const totalDuration = Math.min(audioBuffer.duration * 1000, 20000); // max 20s
...
async function loadSettings() {
  ...
  document.getElementById('enable-toggle').checked = data.enabled !== false; // fallback if not present
  ...
  document.getElementById('threshold-slider').value = data.threshold || 0.82;
  document.getElementById('threshold-value').textContent = (data.threshold || 0.82).toFixed(2);
  
  document.getElementById('sensitivity-slider').value = data.sensitivity || 0.50;
  document.getElementById('sensitivity-value').textContent = (data.sensitivity || 0.50).toFixed(2);
...
function renderSamplesList() {
  ...
  if (currentSamples.length === 0) {
    container.innerHTML = `<div style="text-align:center;padding:20px;color:#666;font-style:italic;">No hay muestras.<br>Agrega clips de voz limpia.</div>`;
    return;
  }
  currentSamples.forEach((sample, index) => {
    ...
    div.innerHTML = `
      <div>
        ...
        <span style="font-size:11px;color:#00ff9d;margin-left:12px;">${sample.duration ? Math.round(sample.duration)+'s' : ''}</span>
      </div>
      <button onclick="deleteSample(${index});" style="background:none;border:none;color:#ff5555;font-size:18px;cursor:pointer;">×</button>
    `;
...
document.getElementById('clear-samples').addEventListener('click', async () => {
  if (confirm('¿Borrar TODAS las muestras de voz?')) {
    currentSamples = [];
...
document.getElementById('sample-upload').addEventListener('change', async (e) => {
  ...
  progressMsg.textContent = '🔄 Procesando muestras...';
...
async function recomputeReferenceSpectrum() {
  ...
  console.log(`%c✅ ${currentSamples.length} muestras actualizadas`, 'color:#00ff9d');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Enable toggle
  const enableOn = document.getElementById('enable-on');
  const enableOff = document.getElementById('enable-off');
  enableOn.addEventListener('click', () => {
    chrome.storage.local.set({ enabled: true });
    updateToggleUI(true);
    chrome.runtime.sendMessage({ target: 'content', action: 'toggle-extension' });
  });
  enableOff.addEventListener('click', () => {
  ...
  // Mode toggles
  document.getElementById('mode-mute').addEventListener('click', () => {
    document.getElementById('mode-mute').classList.add('active');
    document.getElementById('mode-reduce').classList.remove('active');
    chrome.storage.local.set({ mode: 'mute' });
    chrome.runtime.sendMessage({ target: 'content', action: 'toggle-mode' });
  });
  document.getElementById('mode-reduce').addEventListener('click', () => {
    document.getElementById('mode-mute').classList.remove('active');
    document.getElementById('mode-reduce').classList.add('active');
    chrome.storage.local.set({ mode: 'reduction' });
    chrome.runtime.sendMessage({ target: 'content', action: 'toggle-mode' });
  });
  ...
  sensitivitySlider.addEventListener('input', () => {
    ...
    document.getElementById('sensitivity-value').textContent = val.toFixed(2);
  ...
  reductionSlider.addEventListener('input', () => {
    const val = parseFloat(reductionSlider.value);
    document.getElementById('reduction-value').textContent = val + '%';
    chrome.storage.local.set({ reductionPercent: val });
  });

  chrome.storage.onChanged.addListener(() => loadSettings());

  console.log('%c✅ Kick Voice Guard v3.1 - TODOS LOS BOTONES Y SLIDERS FUNCIONAN', 'color:#00ff9d;font-weight:bold');
});
...
function updateLiveStatus() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs[0] && tabs[0].url.includes('kick.com')) {
      document.getElementById('live-indicator').innerHTML = `<span style="color:#00ff9d;">🟢 LIVE en Kick.com</span>`;
    }
  });
}
